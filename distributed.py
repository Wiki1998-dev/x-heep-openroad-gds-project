...
def read_config(file_name):
    ...
    def read_tune_ax(name, this):
        ...
        if min_ == max_:
            ...
            if this['step'] == 1:
                dict_["type"] = "choice"
                dict_["values"] = tune.choice(np.asarray.tolist(
                    np.arange(min_,
                              max_,
                              this['step'])))
...
# Regular helper function to convert numpy objects to native Python types
...
def save_best(results):
    ...
    # Convert numpy types to native Python types before writing to JSON
    best_config = {k: convert_to_native(v) for k, v in best_config.items()}
